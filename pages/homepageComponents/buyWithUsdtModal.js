 ...
 useEffect(() => {
   if (buyWaitSuccess) {
       ...
       // --- Referral Logic (Add this) ---
       const sendPurchaseInfo = async () => {
           try {
               const storedRefCode = Cookies.get('refCode'); // Get from cookie!
               const response = await fetch('/api/purchase', {
                   ...
                   body: JSON.stringify({
                       buyerWallet: address,
                       usdtAmount: usdt,  // Make sure this is the *actual* USDT amount
                       refCode: storedRefCode, // Use stored code
                   }),
               });
               ...
               if (!response.ok) {
                   ...
                   // Handle the error appropriately (show message to user)
               }
           } catch (error) {
       ...
       sendPurchaseInfo(); // Call the function
       // --- End Referral Logic ---
   }

 }, [buyWaitSuccess, buyWithUsdtReset, address, usdt]); // Correct dependencies
 
 // Calculate USDT equivalent and update input box styles
 useEffect(() => {
 ...
 }, [tokens, pricePerToken, usdtBalance]); // Depend on pricePerToken
 ...
 // Set loading text based on transaction state
 ...
 setConvertToUsdtInProcessText(undefined); // Use undefined or null, not an empty string.
 }
